...
import java.awt.event.*;
	...
	/** Number of turns */
		...
		//this extended version consists of sound effects and labels appearing after winning or losing the game.
		...
		private void startPlaying() {
			...
			while (true) {
				if ( ball.getX() <= 0 || ball.getX() >= (WIDTH - 2 * BALL_RADIUS) && getCollidingObject() == null && turns != 0 && getElementCount() > 2 ) {
					vx = -vx;
				} else if ( ball.getY() <= 0 && turns != 0 && getElementCount() > 2 ) {
					vy = -vy;	
				} else if ( getCollidingObject() != paddle & getCollidingObject() != null && turns != 0 && getElementCount() > 2 ) {
					remove(getCollidingObject());
					bounceClip.play();
					vy = -vy;
				} else if ( ball.getY() >= getHeight() - 2 * BALL_RADIUS && turns != 0 && getElementCount() > 2 ) {
					turns -=1;
					ball.setLocation(WIDTH/2 - BALL_RADIUS, getHeight()/2 - BALL_RADIUS);
					pause(500);
				} else if (turns == 0) {
					GLabel loseLabel = new GLabel ("YOU LOSE!");
					loseLabel.setFont("London-36"); 
					loseLabel.setColor(Color.RED); 
					add (loseLabel, (WIDTH - loseLabel.getWidth())/2.0, 3*(getHeight() - (loseLabel.getHeight()))/4.0);
					break;
				} else if (getCollidingObject() == paddle && turns != 0) {
					vy = -vy;
					bounceClip.play();
				} else if (getElementCount() <= 2) {
					GLabel winLabel = new GLabel ("YOU WIN!");
					winLabel.setFont("London-36"); 
					winLabel.setColor(Color.GREEN); 
					add (winLabel, (WIDTH - winLabel.getWidth())/2.0, 3*(getHeight() - (winLabel.getHeight()))/4.0);
					break;
				}
				if (turns != 0) {
					ball.move(vx, vy);
